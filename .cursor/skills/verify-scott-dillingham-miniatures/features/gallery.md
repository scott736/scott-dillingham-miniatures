...
- `gallery-card` shows title, truncated description, and `Read Similar Story`.
...
- **Piece card.** The first card is `article#tall-case-clock`. Its title is a `<button data-gallery-open="tall-case-clock">` whose name is `Simon Willard Tall Case Clock Style`. The card description is truncated static text. There is no `Read More` on the card. `Read Similar Story` goes to `/blog/miniature-tall-case-clocks`.
- **Open lightbox.** Click the title button `Simon Willard Tall Case Clock Style`. A dialog appears with heading `Simon Willard Tall Case Clock Style`, a close button named `Close`, and `Read More` when the description is longer than 225 characters. The card page URL stays `/gallery`.
...
- `GalleryLightboxIsland` is `client:idle`. Card titles are in prerendered HTML; the lightbox dialog needs JS.
...
- `Read More` / `Read Less` live in the lightbox `ExpandableDescription`, not on the card. They appear when the stripped description is longer than 225 characters (all current pieces).
- The card root is an `<article id>`, not a `div`. Assert `id="tall-case-clock"` or `article#tall-case-clock`.
