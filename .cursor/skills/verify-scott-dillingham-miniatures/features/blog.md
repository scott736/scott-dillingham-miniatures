...
- `blog-search` filters titles and descriptions through `input[data-blog-search]` (`Search articles...`).
- `blog-tag` filters by a tag chip (for example `1/12 scale`) or `All`.
- `blog-empty` shows `No articles found.` when zero cards remain.
...
- **Open index.** Run `curl -sS -D evidence/blog/before.headers.txt -o evidence/blog/before.html http://127.0.0.1:4318/blog`. Status `200`. Body contains visible `h1` `From the Workshop Journal`, `<title>` `Miniature Furniture Blog`, `data-blog-search`, `placeholder="Search articles..."`, tag chip `1/12 scale`, and a link to `/blog/complete-guide-1-12-scale-miniature-furniture`.
...
- **Search (browser only).** On `/blog`, fill `input[data-blog-search]` with `tall case`. The card `Miniature Tall Case Clocks: The Ultimate Challenge` remains. Fill `volcano`. `[data-blog-empty]` becomes visible with exact text `No articles found.` (no query echo). Clear by emptying the input or choosing tag `All`. The full grid returns.
- **Proof.** `before.html` is the index. `after.html` is the article with the H1 and an in-body heading. Record feature id `blog`.

## Gotchas

- Search and tags are client state. `curl /blog` always returns the full list, including the hidden `No articles found.` node. Empty-state proof needs a browser.
- The index H1 is visible `From the Workshop Journal`. `Miniature Furniture Blog` is the document title, not an `sr-only` heading.
- There is no `Clear search` control and no `Clear filters` button. Empty the search field or choose `All`.
